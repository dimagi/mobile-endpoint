...
import logging
from mobile_endpoint.case.xml import V2_NAMESPACE, V2, V1, check_version
from mobile_endpoint.utils import json_format_date, json_format_datetime
...
class V1CaseXMLGenerator(CaseXMLGeneratorBase):
    ...
    def get_root_element(self):
        ...
        root.append(safe_element("case_id", self.case.id))
    ...
    def add_indices(self, element):
        ...
        if self.case.indices:
            logging.info("Tried to add indices to version 1 CaseXML restore. This is not supported. "
                         "The case id is %s, domain %s." % (self.case.id, self.case.domain))
